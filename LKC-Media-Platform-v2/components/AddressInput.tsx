...
export default function AddressInput({ onLocationChange }: AddressInputProps) {
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      >
        Cannot find it? Enter the location manually
      </button>
      {selected && (
        <>
          ...
          <p className="mt-2 text-xs text-[#0088ff]">✓ Location selected</p>
        </>
      )}

      <p className="mt-2 text-xs leading-5 text-white/30">
        Search for a school, stadium, park, field, venue, or address in Arizona.
      </p>
    </div>
  );
}
