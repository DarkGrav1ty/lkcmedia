...
export default async function AlbumPhotos({
  ...
  const { data, error, count } = await getSupabaseAdmin()
    .from("media_assets")
    .select("id,file_name,alt_text,width,height", { count: "exact" })
    .eq("album_id", id)
    .eq("is_visible", true)
  ...
  if (error) throw new Error("Gallery photos unavailable");
  return (
    <>
      ...
      <GalleryLightbox
        photos={(data || []).map((m) => ({
          ...
          height: m.height,
          ...(privateGallery
            ? { download: `/api/client/download/${m.id}` }
            : {}),
        }))}
      />
      <Pagination page={page} total={count || 0} path={path} />
    </>
  );
}
