...
export async function GET(
  ...
  try {
    ...
    if (mediaError || !media || !media.album_id || !media.preview_path) {
      return fail("Image unavailable.", 404);
    }

    const { data: album, error: albumError } = await db
      .from("albums")
      .select("id,is_visible,is_private,expires_at")
    ...
    if (
      album.expires_at &&
      new Date(album.expires_at).getTime() <= Date.now()
    ) {
      return fail("Image unavailable.", 404);
    }
    ...
    const path =
      size === "thumb" && media.thumbnail_path
